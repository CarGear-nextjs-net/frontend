...
export const getAllCategoryApi = async () => {
  const res = await axios.get(`${API_BASE_URL}/api/CategoryManager/categories`);
  return res;
};
...
export const getCategoryApiDetail = async (id) => {
  const res = await axios.get(`${API_BASE_URL}/api/categorymanager/${id}`);
  return res;
};
