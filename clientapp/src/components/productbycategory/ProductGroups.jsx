...
import { Star, ShoppingCart, ChevronRight, Eye, Heart } from 'lucide-react'
import Image from "next/image"
import Link from "next/link"
import {formatPrice} from "@/utils/format";

export default function ProductGroups({ groupProductMap = [] }) {
    ...
    return (
        <div className="py-12 px-4">
            <div className="container mx-auto">
                {groupProductMap.map((item) => (
                    item.products.length > 0 && (
                        <div key={item.group.id} className="mb-12 ">
                            {/* Group Header */}
                            <div className="flex items-center justify-between mb-6">
                                <div className="flex items-center">
                                    <div className="w-1 h-8 bg-red-600 mr-3"></div>
                                    <div>
                                        <h2 className="text-xl md:text-2xl font-bold text-gray-800">{item.group.name.toUpperCase()}</h2>
                                        <p className="text-sm text-gray-500 hidden md:block">{item.group.description}</p>
                                    </div>
                                </div>
                                <Link
                            ...
                            <div className="mt-4 text-center md:hidden">
                                ...
                                >
                                    Xem tất cả {item.group.name.toLowerCase()} <ChevronRight className="h-4 w-4 ml-1" />
                                </Link>
                            </div>
                        </div>
                    )
